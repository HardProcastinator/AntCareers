...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'post_job') {
        ...
        try {
            $db = getDB();
            $db->prepare("INSERT INTO jobs (employer_id,title,description,requirements,location,job_type,setup,salary_min,salary_max,salary_currency,industry,experience_level,skills_required,status,deadline,country,recruitment_duration) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'Active',?,?,?)")
    ...
    if ($action === 'update_job') {
        ...
        $currency = $country ? (COUNTRY_CURRENCIES[$country] ?? 'PHP') : 'PHP';
        try {
            getDB()->prepare("UPDATE jobs SET title=?,description=?,requirements=?,location=?,job_type=?,setup=?,salary_min=?,salary_max=?,salary_currency=?,industry=?,experience_level=?,skills_required=?,deadline=?,country=?,recruitment_duration=? WHERE id=? AND employer_id=?")
...
<head>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
    ...
    @media(max-width:480px){.frow[style*="grid-template-columns"]{grid-template-columns:1fr!important;}}
...
<div class="modal-bd" id="jobModal">
  <div class="modal-box">
    ...
    <div class="frow" style="grid-template-columns:1fr 1fr 1fr;">
      <div class="fg"><label class="fl">Country</label><select class="fi" id="fCountry" onchange="onCountryChange()"><option value="">— Select Country —</option><?php foreach (getCountries() as $c): ?><option value="<?= htmlspecialchars($c['code']) ?>"><?= htmlspecialchars($c['name']) ?></option><?php endforeach; ?></select></div>
      <div class="fg"><label class="fl">Location</label><input type="text" class="fi" id="fLoc" placeholder="City, Province…"></div>
    ...
    <div class="frow">
      <div class="fg"><label class="fl" id="lblSMin">Min Salary (PHP)</label><input type="number" class="fi" id="fSMin" placeholder="e.g. 50000"></div>
      <div class="fg"><label class="fl" id="lblSMax">Max Salary (PHP)</label><input type="number" class="fi" id="fSMax" placeholder="e.g. 90000"></div>
    </div>
    <div class="frow">
      <div class="fg"><label class="fl">Application Deadline</label><input type="date" class="fi" id="fDl"></div>
      <div class="fg"><label class="fl">Recruitment Duration</label><select class="fi" id="fDuration"><option value="">— Not specified —</option><option value="1 week">1 Week</option><option value="2 weeks">2 Weeks</option><option value="1 month">1 Month</option><option value="2 months">2 Months</option><option value="3 months">3 Months</option><option value="6 months">6 Months</option><option value="Ongoing">Ongoing</option></select></div>
    </div>
...
<script>
/* Country → Currency map */
const COUNTRY_CURRENCY={'PH':'PHP','US':'USD','GB':'GBP','AU':'AUD','CA':'CAD','JP':'JPY','KR':'KRW','SG':'SGD','HK':'HKD','MY':'MYR','TH':'THB','ID':'IDR','IN':'INR','CN':'CNY','NZ':'NZD','AE':'AED','SA':'SAR','DE':'EUR','FR':'EUR','IT':'EUR','ES':'EUR','NL':'EUR','IE':'EUR','PT':'EUR','AT':'EUR','BE':'EUR','FI':'EUR','GR':'EUR','LU':'EUR','MT':'EUR','CY':'EUR','EE':'EUR','LV':'EUR','LT':'EUR','SK':'EUR','SI':'EUR','HR':'EUR','BG':'BGN','RO':'RON','PL':'PLN','CZ':'CZK','HU':'HUF','SE':'SEK','DK':'DKK','NO':'NOK','CH':'CHF','BR':'BRL','MX':'MXN','ZA':'ZAR','NG':'NGN','EG':'EGP','KE':'KES','GH':'GHS','TW':'TWD','VN':'VND','PK':'PKR','BD':'BDT','LK':'LKR','NP':'NPR','QA':'QAR','KW':'KWD','BH':'BHD','OM':'OMR','JO':'JOD','IL':'ILS','TR':'TRY','RU':'RUB','UA':'UAH','CL':'CLP','CO':'COP','PE':'PEN','AR':'ARS'};
function onCountryChange(){var code=document.getElementById('fCountry').value;var cur=COUNTRY_CURRENCY[code]||'PHP';document.getElementById('lblSMin').textContent='Min Salary ('+cur+')';document.getElementById('lblSMax').textContent='Max Salary ('+cur+')';}
  var isEdit=false;
  ...
  function clearForm(){['fLoc','fSMin','fSMax','fSkills','fDesc','fReq','fDl','fTitleCustom','fDuration'].forEach(function(id){document.getElementById(id).value='';});document.getElementById('fTitleCustom').style.display='none';document.getElementById('fType').value='Full-time';document.getElementById('fSetup').value='On-site';document.getElementById('fExp').value='';document.getElementById('fInd').value='';updateJobTitleOptions();document.getElementById('fCountry').value='';onCountryChange();}
  function editJob(id){doPost({action:'get_job',job_id:id},function(d){if(!d.ok){toast(d.msg||'Error','err');return;}var j=d.job;isEdit=true;document.getElementById('eJobId').value=j.id;document.getElementById('fDesc').value=j.description||'';document.getElementById('fReq').value=j.requirements||'';document.getElementById('fLoc').value=j.location||'';document.getElementById('fCountry').value=j.country||'';onCountryChange();document.getElementById('fDuration').value=j.recruitment_duration||'';document.getElementById('fType').value=j.job_type||'Full-time';document.getElementById('fSetup').value=j.setup||'On-site';document.getElementById('fSMin').value=j.salary_min||'';document.getElementById('fSMax').value=j.salary_max||'';document.getElementById('fInd').value=j.industry||'';updateJobTitleOptions();var _knownTitle=document.getElementById('fTitle');var _titleVal=j.title||'';var _opts=[..._knownTitle.options].map(function(o){return o.value;});if(_titleVal&&!_opts.includes(_titleVal)&&_titleVal!==''){_knownTitle.value='Other';var _ci=document.getElementById('fTitleCustom');_ci.style.display='block';_ci.value=_titleVal;}else{_knownTitle.value=_titleVal;toggleCustomTitle();}document.getElementById('fExp').value=j.experience_level||'';document.getElementById('fSkills').value=j.skills_required||'';document.getElementById('fDl').value=j.deadline||'';document.getElementById('mTitle').innerHTML='<i class="fas fa-edit" style="color:var(--red-bright)"></i> Edit Job';document.getElementById('submitBtn').innerHTML='<i class="fas fa-save"></i> Save Changes';document.getElementById('jobModal').classList.add('open');});}
  function submitJob(){var sel=document.getElementById('fTitle');var t=(sel.value==='Other'?document.getElementById('fTitleCustom').value:sel.value).trim();if(!t){toast('Job title required','err');return;}var id=document.getElementById('eJobId').value;var data={action:id?'update_job':'post_job',title:t,description:document.getElementById('fDesc').value,requirements:document.getElementById('fReq').value,location:document.getElementById('fLoc').value,job_type:document.getElementById('fType').value,setup:document.getElementById('fSetup').value,salary_min:document.getElementById('fSMin').value,salary_max:document.getElementById('fSMax').value,industry:document.getElementById('fInd').value,experience_level:document.getElementById('fExp').value,skills:document.getElementById('fSkills').value,deadline:document.getElementById('fDl').value,country:document.getElementById('fCountry').value,recruitment_duration:document.getElementById('fDuration').value};if(id)data.job_id=id;doPost(data,function(d){if(d.ok){closeJobModal();toast(id?'Job updated!':'Job "'+d.title+'" posted!','ok');setTimeout(function(){location.reload();},1200);}else{toast(d.msg||'Error','err');}});}
