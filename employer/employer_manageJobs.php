...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    /* ── post_job (Active + pending approval) ── */
    if ($action === 'post_job') {
    ...
    if ($action === 'save_draft') {
        ...
        $currency = $country ? (COUNTRY_CURRENCIES[$country] ?? 'PHP') : 'PHP';
        try {
            $db->prepare("INSERT INTO jobs (employer_id, title, description, requirements, location, job_type, setup, salary_min, salary_max, salary_currency, industry, experience_level, skills_required, status, approval_status, deadline, country, recruitment_duration) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'Draft','pending',?,?,?)")
               ->execute([$uid, $title, $desc, $req, $loc, $type, $setup, $sMin, $sMax, $currency, $ind, $exp, $skills, $dl, $country, $duration]);
            echo json_encode(['ok' => true, 'job_id' => (int)$db->lastInsertId(), 'title' => $title]);
        } catch (Exception $e) {
            error_log('[AntCareers] employer save_draft: ' . $e->getMessage());
            echo json_encode(['ok' => false, 'msg' => 'DB error']);
        }
        exit;
    }
    ...
    if ($action === 'update_job') {
        ...
        $currency = $country ? (COUNTRY_CURRENCIES[$country] ?? 'PHP') : 'PHP';
        try {
            $db->prepare("UPDATE jobs SET title=?, description=?, requirements=?, location=?, job_type=?, setup=?, salary_min=?, salary_max=?, salary_currency=?, industry=?, experience_level=?, skills_required=?, deadline=?, country=?, recruitment_duration=?, updated_at=NOW() WHERE id=? AND employer_id=?")
            ...
            error_log('[AntCareers] employer update_job: ' . $e->getMessage());
            echo json_encode(['ok' => false, 'msg' => 'DB error']);
        }
        exit;
    }
    ...
    if ($action === 'post_draft') {
        $jobId = (int)($_POST['job_id'] ?? 0);
        try {
            $st = $db->prepare("UPDATE jobs SET status='Active', approval_status='pending', updated_at=NOW() WHERE id=? AND employer_id=? AND status='Draft'");
            $st->execute([$jobId, $uid]);
            if ($st->rowCount() === 0) {
            ...
            error_log('[AntCareers] employer post_draft: ' . $e->getMessage());
            echo json_encode(['ok' => false, 'msg' => 'DB error']);
        }
        exit;
    }
    ...
    if ($action === 'toggle_status') {
        $jobId = (int)($_POST['job_id'] ?? 0);
        try {
            $row = $db->prepare("SELECT status, approval_status FROM jobs WHERE id=? AND employer_id=?");
    ...
    if ($action === 'get_job') {
        $jobId = (int)($_POST['job_id'] ?? 0);
        try {
            $st = $db->prepare("SELECT * FROM jobs WHERE id=? AND employer_id=?");
            $st->execute([$jobId, $uid]);
            $r = $st->fetch(PDO::FETCH_ASSOC);
            if (!$r) { echo json_encode(['ok' => false, 'msg' => 'Not found']); exit; }
            echo json_encode(['ok' => true, 'job' => $r]);
        } catch (Exception $e) {
            error_log('[AntCareers] employer get_job: ' . $e->getMessage());
            echo json_encode(['ok' => false, 'msg' => 'DB error']);
        }
        exit;
    }
...
<head>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
    ...
    /* ── Navbar base (employer navbar doesn't self-style) ── */
    ...
    body.light .theme-btn,body.light .notif-btn-nav,body.light .profile-btn{background:#F5EEEC;border-color:#E0CECA;}
    ...
    .page-title{font-family:var(--font-display);font-size:28px;font-weight:700;color:var(--text-light);margin-bottom:4px;}
    ...
    .job-title{font-family:var(--font-display);font-size:18px;font-weight:700;color:var(--text-light);margin-bottom:6px;letter-spacing:-0.01em;}
    ...
    .app-count{font-family:var(--font-display);font-size:22px;font-weight:700;color:var(--text-light);text-align:center;}
    ...
    .modal-title{font-family:var(--font-display);font-size:21px;font-weight:700;color:var(--text-light);margin-bottom:20px;display:flex;align-items:center;gap:9px;}
    ...
    .fi{width:100%;padding:10px 14px;border-radius:8px;background:var(--soil-hover);border:1px solid var(--soil-line);color:var(--text-light);font-family:var(--font-body);font-size:13px;outline:none;transition:all .2s;}
    ...
    .confirm-title{font-family:var(--font-display);font-size:18px;color:var(--text-light);margin-bottom:7px;}
    ...
    .inv-modal-title{font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--text-light);line-height:1.2;}
    ...
    .inv-search-wrap input{width:100%;padding:10px 14px 10px 34px;border-radius:9px;background:var(--soil-hover);border:1px solid var(--soil-line);color:var(--text-light);font-family:var(--font-body);font-size:13px;outline:none;transition:.2s;}
    ...
    .inv-seeker-name{font-size:13px;font-weight:600;color:var(--text-light);display:flex;align-items:center;gap:5px;flex-wrap:wrap;}
    ...
    .inv-preview-greeting{font-size:15px;font-weight:700;color:var(--text-light);margin-bottom:14px;font-family:var(--font-display);}
