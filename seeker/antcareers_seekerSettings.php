...
// Convenience aliases (same pattern as all other seeker pages)
...
settings_ensure_column($db, 'seeker_profiles', 'show_in_people_search', 'TINYINT(1) NOT NULL DEFAULT 1');
...
$visStmt->execute([(int)$user['id']]);
if ($visRow = $visStmt->fetch(PDO::FETCH_ASSOC)) {
  $profileVisibility = (int)($visRow['show_in_people_search'] ?? 1);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'change_password') {
    ...
    if ($passwordHash === '' || !password_verify($currentPassword, $passwordHash)) {
      header('Location: antcareers_seekerSettings.php?error=bad_password');
      exit;
    }
    if (strlen($newPassword) < 8 || $newPassword !== $confirmPassword) {
      header('Location: antcareers_seekerSettings.php?error=password_mismatch');
      exit;
    }
    ...
    header('Location: antcareers_seekerSettings.php?pw=1');
    exit;
  }
  ...
  if ($action === 'save_preferences') {
    ...
    $visUpdate->execute([
      ':show' => $showInPeopleSearch,
      ':uid' => (int)$user['id'],
    ]);

    header('Location: antcareers_seekerSettings.php?prefs=1');
    exit;
  }
...
<head>
  ...
  <style>
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .sb-badge.green { background:var(--green); color:#fff; }
    .sb-browse-wrap { padding:12px 14px; border-top:1px solid var(--soil-line); background:var(--soil-card); flex-shrink:0; }
    .sb-browse { display:flex; align-items:center; justify-content:center; gap:8px; padding:11px; border-radius:8px; background:var(--red-vivid); border:none; color:#fff; font-family:var(--font-body); font-size:13px; font-weight:700; cursor:pointer; transition:all 0.2s; width:100%; box-shadow:0 2px 12px rgba(209,61,44,0.3); text-decoration:none; }
    ...
    /* Avatar display in account info */
    ...
    .btn-danger { padding:10px 22px; border-radius:8px; background:transparent; border:1px solid rgba(224,85,85,0.3); color:#E05555; font-family:var(--font-body); font-size:13px; font-weight:700; cursor:pointer; transition:0.2s; display:inline-flex; align-items:center; gap:8px; }
  ...
  <div class="content-layout">
    ...
    <div class="settings-content">
      ...
      <div class="section-card anim anim-d3">
        ...
        <form class="section-body" id="passwordForm" method="post" action="antcareers_seekerSettings.php">
          ...
          <div class="form-group">
            ...
            <div class="form-input-wrap">
              ...
              <button type="button" class="toggle-pw" tabindex="-1" onclick="togglePw('currentPassword', this)" aria-label="Show/hide">
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <div class="form-input-wrap">
                ...
                <button type="button" class="toggle-pw" tabindex="-1" onclick="togglePw('newPassword', this)" aria-label="Show/hide">
                  <i class="fas fa-eye"></i>
                </button>
              </div>
            ...
            <div class="form-group">
              ...
              <div class="form-input-wrap">
                ...
                <button type="button" class="toggle-pw" tabindex="-1" onclick="togglePw('confirmPassword', this)" aria-label="Show/hide">
                  <i class="fas fa-eye"></i>
                </button>
              </div>
            </div>
      ...
      <div class="section-card anim anim-d5">
        ...
        <div class="section-body">
          ...
          <div class="security-row">
            ...
            <form method="post" action="antcareers_seekerSettings.php" onsubmit="return confirm('Deactivate your account?');">
...
(function () {
  ...
  // ── SYNC dark mode toggle with the existing theme system ──────────────────
  function syncDarkToggle() {
    const isDark = !document.body.classList.contains('light');
    const toggle = document.getElementById('darkModeToggle');
    if (toggle) toggle.checked = isDark;
  }
  // Run on load after navbar script has applied theme
  ...
  window.handleThemeToggle = function (isDark) {
    // Reuse the navbar's applyTheme if available, otherwise do it ourselves
    ...
    showToast(isDark ? 'Dark mode enabled' : 'Light mode enabled', 'fa-adjust');
  };
  ...
  // ── SAVE PASSWORD (UI only) ───────────────────────────────────────────────
  window.handleSavePassword = function () {
    const cur  = document.getElementById('currentPassword').value.trim();
    const nw   = document.getElementById('newPassword').value;
    const conf = document.getElementById('confirmPassword').value;

    if (!cur) { showToast('Please enter your current password', 'fa-exclamation-circle'); return; }
    if (nw.length < 8) { showToast('New password must be at least 8 characters', 'fa-exclamation-circle'); return; }
    if (nw !== conf)   { showToast('Passwords do not match', 'fa-exclamation-circle'); return; }
